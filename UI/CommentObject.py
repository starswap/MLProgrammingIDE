...
class Comment(PyQt5.QtWidgets.QWidget):
	...
	def __init__(self,commentText,parentLayout):
		...
		self.ui.dismissButton.clicked.connect(self.die) #When the user closes the comment it will need to disappear
		
		parentLayout.insertWidget(0,self) #Insert into parent at the top so it displays on screen in the correct place
	
	def die(self):
		"""Called when the user clicks the Dismiss Suggestion button"""
		self.setParent(None) #Dissociate from parent so we can safely delete it
		self.deleteLater() #Allow Qt Windowing system to decide the best moment to remove it from memory and call the destructor (which we don't have access to as it's C++)
